...
def upload_image(image: schemas.UploadImageSchema, db: Session = Depends(get_db),
                 owner_id: str = Depends(require_user)):
    image.user_id = uuid.UUID(owner_id)
...
def update_image(id: str, image: schemas.UpdateImageSchema, db: Session = Depends(get_db),
                 user_id: str = Depends(require_user)):
